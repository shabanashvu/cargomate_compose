...
class Settlement(models.Model):
    ...
    invoice = fields.Many2one(
        comodel_name="account.move", string="Generated Bill",
        readonly=True)
    ...
    def _prepare_invoice_header(self, settlement, journal, date=False):
        invoice = self.env['account.move'].new({
            ...
            'move_type': ('in_invoice' if journal.type == 'purchase' else
                     'in_refund'),
            'invoice_date': date,
...
class SettlementLine(models.Model):
    ...
    invoice_line = fields.Many2one(
        comodel_name='account.move.line', store=True)
    # invoice_line = fields.Many2one(
    #     comodel_name='account.invoice.line', store=True,
    #     related='agent_line.object_id')
    invoice = fields.Many2one(
        comodel_name='account.move', store=True, string="Invoice",
    ...
    currency_id = fields.Many2one(
        related="invoice_line.currency_id",
        store=True,
        readonly=True,
    )
    commission = fields.Many2one(
        comodel_name="sale.commission", related="agent.commission")
    company_id = fields.Many2one(
        comodel_name='res.company',
